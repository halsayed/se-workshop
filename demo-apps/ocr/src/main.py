...
def main():
    ...
    with left_col:
        ...
        with st.container():
            ...
            base_url = "https://ai.gpt.ntnx.pro/api/v1"
            api_url = st.text_input('API Endpoint URL', value=config('API_ENDPOINT', default='https://ai.gpt.ntnx.pro/api/v1'))
            
            if api_url and not is_valid_url(api_url):
                st.error("Please enter a valid URL")
            
            model_name = st.text_input('Model Name', value=config('VISION_MODEL_NAME', default='vision'))
